...
<hr />
     ...
     <div>
      ...
      <h2 class="text-center" style="text-decoration:underline;">Dossier médical <h2><h2 class="text-center"><?php if($donneesRequete1['Sexe']='M'){echo 'M. ';} else {echo 'Mme. ';} echo $donneesRequete1['Nom'].' '.$donneesRequete1['Prenoms'];?> </h2>
       <br />
      ...
      &nbsp;&nbsp;<strong style="">Adresse</strong> <?php echo ': '.$donneesRequete1['Adresse'];?>
      
      <br />
       
      &nbsp;&nbsp;<strong style="">Pays d'origine</strong> <?php echo ': '.$donneesRequete1['PaysOrigine'];?>
      
      <br />
       
      &nbsp;&nbsp;<strong style="">Profession</strong> <?php echo ': '.$donneesRequete1['Profession'];?>
      <br />
       
      &nbsp;&nbsp;<strong style="">Groupe sanguin</strong> <?php echo ': '.$donneesRequete1['GroupeSanguin'];?>
      
      <br />
       
      &nbsp;&nbsp;<strong style="">Allergies</strong> <?php echo ': '.$donneesRequete1['Allergies'];?>
      <br />
       
      &nbsp;&nbsp;<strong style="">Antécédents personnels</strong> <?php echo ': '.$donneesRequete1['AntecedentsPersonnels'];?>
      
      <br />
       
      &nbsp;&nbsp;<strong style="">Antécédents familiaux</strong> <?php echo ': '.$donneesRequete1['AntecedentsFamiliaux'];?>
      
      <br />
      <br />
       
      &nbsp;&nbsp;<strong style="">Personne à prevenir en cas d'urgence</strong> <?php echo '<br/>&nbsp;&nbsp;&nbsp;&nbsp;Nom : '.$donneesRequete1['PapNom'].'<br/>&nbsp;&nbsp;&nbsp;&nbsp;Prenoms : '.$donneesRequete1['PapPrenoms'].'<br/>&nbsp;&nbsp;&nbsp;&nbsp;Tel Domicile : '.$donneesRequete1['PapTelephoneDomicile']; if(!empty($donneesRequete1['PapTelephoneProfessionnel'])) echo ' <br/>&nbsp;&nbsp;&nbsp;&nbsp;Tel Professionnel : '.$donneesRequete1['PapTelephoneProfessionnel']; if(!empty($donneesRequete1['PapEmail'])) echo ' <br/>&nbsp;&nbsp;&nbsp;&nbsp;E-Mail : '.$donneesRequete1['PapEmail'];?>
    ...
    <?php  
                $reponseCalculConst=$bdd->prepare('SELECT * FROM RELEVER_PARAMETRE WHERE Per_Id=:Per_Id ORDER BY Date DESC'); 
...
<hr />
  ...
  <?php $reponseCalcul2=$bdd->prepare('SELECT * FROM CONSULTATION WHERE Per_Id=:Per_Id'); 
                $reponseCalcul2->execute(array('Per_Id'=>$_POST['Id']));
				
                $total2=count($reponseCalcul2->fetchAll());
				   
                $reponse=$bdd->prepare('SELECT Id, Per_Id, CodeConsultation, Plaintes, ObservationsEtExamenClinique, DiagnosticClinique, Diagnostic, Remarques, Issue, DATE_FORMAT(DateFinConsultation, \'%d/%m/%Y \à %H:%i:%S\') AS dateFin, DATE_FORMAT(DateDebutConsultation, \'%d/%m/%Y \à %H:%i:%S\') AS dateDebut, DATE_FORMAT(DateDebutConsultation, \'%d-%m-%Y-%H-%i-%S\') AS dateUnique, DateDebutConsultation, Issue FROM CONSULTATION WHERE Per_Id=:Per_Id ORDER BY dateDebut'); 
...
$reponseCalcul4=$bdd->prepare('SELECT * FROM HOSPITALISER WHERE Per_Id=:Per_Id'); 
                $reponseCalcul4->execute(array('Per_Id'=>$_POST['Id']));
				
                $total4=count($reponseCalcul4->fetchAll());
				   
                $reponse4=$bdd->prepare('SELECT NumLit, Id, Per_Id, Motif, PremiersSoins, DATE_FORMAT(DateFinHospitalisation, \'%d/%m/%Y \à %H:%i:%S\') AS dateFin, DATE_FORMAT(Date, \'%d/%m/%Y \à %H:%i:%S\') AS dateDebut, DATE_FORMAT(Date, \'%d-%m-%Y-%H-%i-%S\') AS dateUnique, Date FROM HOSPITALISER WHERE Per_Id=:Per_Id ORDER BY dateDebut DESC'); 
