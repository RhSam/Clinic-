...
<title>ordonnance du <?php echo date('Y-m-d H:i:s');?> </title>
...
<?php //sécurité contre l'accès à la page par l'url directement
 ...
 $requeteOrdonnance=$bdd->prepare('SELECT * FROM ENREGISTRER_PRODUIT WHERE Date>=:Date ORDER BY Date');
 $requeteOrdonnance->execute(array('Date'=>str_ireplace('_',' ',$_GET['madate'])));
...
<hr />
     ...
     <div>
      ...
      <h2 class="text-center" style="text-decoration:underline;">Ordonnance médicale</h2> <br /> <h2 class="text-center"><?php if($donneesRequete1['Sexe']=='M'){echo 'M. ';} else {echo 'Mme. ';} echo $donneesRequete1['Nom'].' '.$donneesRequete1['Prenoms'].'  &nbsp; '.Age(dateFr($donneesRequete1['DateDeNaissance'])).' ans';?> </h2>
       <br />
     ...
     <?php 
	 $j=1;
	  while($donneesOrdonnance=$requeteOrdonnance->fetch())
	  {
		  $reponseProduit=$bdd->prepare('SELECT * FROM PRODUIT WHERE IdProduit=:IdProduit'); 
          $reponseProduit->execute(array('IdProduit'=>$donneesOrdonnance['IdProduit']));
		  $donneesProduit=$reponseProduit->fetch();

		  echo $j.' : '.$donneesProduit['NomProduit'].' '.$donneesProduit['FormatProduit'].' '.$donneesProduit['DosageProduit'].'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$donneesOrdonnance['Quantite'].'<br/>'.$donneesOrdonnance['Posologie'].'<br/><br/>';
