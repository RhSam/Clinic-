...
<?php //sécurité contre l'accès à la page par l'url directement
 if ($_SESSION['clearance']<3) //les users avec un niveau inf à 2( secrétaires et externes) ne peuvent accéder à la consultation
 {
 ...
 <ul  class="list-inline">
  ...
  <br />
  <li class="grandchild" id="current">»Anciennes</li>
   ...
   <div class="row row2">
     ...
     <div class="col-xs-12">
        <?php
                ...
                $reponseConst=$bdd->prepare('SELECT Id, Per_Id, PoidsEnKg, TailleEnCm, Temperature, TensionArterielleBG, TensionArterielleBD, Pouls, DATE_FORMAT(Date, \'%d/%m/%Y \à %H:%i:%S\') AS date, Date  FROM RELEVER_PARAMETRE WHERE Per_Id=:Per_Id ORDER BY Date DESC'); 
		...
		?>
                      ...
                      <button class="btn btn-default"><a href="constantesTous.php">
				 ...
				 <table class="table table-bordered table-striped header-fixed4">
                   ...
                   <tbody>
                      ...
                      {      
                          ...
                          echo '<tr>
                                 <td height="70">'.$donneesConst['date'].'</td>
                                 <td height="70" style="overflow:auto;">'; if($donneesResponsable['IdPrivilege']>2)
								 {echo 'Dr. ';} else {echo 'Inf. ';} echo ' '.$donneesResponsable['Nom'].' '.$donneesResponsable['Prenoms'].'</td>
                                 <td height="70">'.$donneesConst['Temperature'].'</td>
								 <td height="70">'.$donneesConst['PoidsEnKg'].'</td> 
                                 <td height="70">';if(!empty($donneesConst['TensionArterielleBG'])){echo '-B.G: '.$donneesConst['TensionArterielleBG'].'</br>';} if(!empty($donneesConst['TensionArterielleBD'])){echo '-B.D: '.$donneesConst['TensionArterielleBD'];} echo '</td> 
								 
                                 <td height="70">'.$donneesConst['TailleEnCm'].'</td>
								 <td height="70">'.$donneesConst['Pouls'].'</td>
                               </tr>';
